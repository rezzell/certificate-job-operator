...
import (
	...
	"github.com/russell/certificate-job-operator/internal/controller"
	webhookv1alpha1 "github.com/russell/certificate-job-operator/internal/webhook/v1alpha1"
)
...
func main() {
	...
	managerOptions := ctrl.Options{
		...
		LeaderElectionID:       "certificate-job-operator.rezzell.com",
	}

	if watchConfig := watchNamespaceCacheConfig(); len(watchConfig) > 0 {
		managerOptions.Cache = cache.Options{DefaultNamespaces: watchConfig}
		setupLog.Info("watching configured namespaces", "namespaces", keys(watchConfig))
	} else {
	...
	if err := webhookv1alpha1.SetupCertificateJobWebhookWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create webhook", "webhook", "CertificateJob")
		os.Exit(1)
	}
...
func keys(in map[string]cache.Config) []string {
	k := make([]string, 0, len(in))
	for key := range in {
		k = append(k, key)
	}
	return k
